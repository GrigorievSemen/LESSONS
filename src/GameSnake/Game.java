...
public class Game extends Application implements GameScreen {
        ...
        for(int y = 0; y < this.height; ++y) {
            for(int x = 0; x < this.width; ++x) {
                ...
                if (this.showGrid && children.size() > 0) {
                    ...
                    cell.setStroke(this.toFXColor(GameSnake.Color.BLACK));
                }
    ...
    private void createScorePanel() {
        ...
        this.scoreText.setFill(this.toFXColor(GameSnake.Color.BLACK));
        StackPane scorePane = new StackPane(new Node[]{this.scoreText});
        scorePane.setBorder(new Border(new BorderStroke[]{new BorderStroke(this.toFXColor(GameSnake.Color.BLACK), BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)}));
        scorePane.setLayoutY((double)(this.height * cellSize + 110 + 6));
        int scoreHeight = 20;
        Rectangle rectangle;
        if (this.showGrid) {
            rectangle = new Rectangle((double)((this.width * cellSize - 1) / 2), (double)scoreHeight, this.toFXColor(GameSnake.Color.WHITE));
            scorePane.setLayoutX((double)(125 + (this.width * cellSize - 1) / 4));
        } else {
            rectangle = new Rectangle((double)(this.width * cellSize / 2), (double)scoreHeight, this.toFXColor(GameSnake.Color.WHITE));
    ...
    public void setCellColor(int x, int y, GameSnake.Color color) {
        if (color != null && color != GameSnake.Color.NONE) {
            ObservableList<Node> children = this.cells[y][x].getChildren();
    ...
    public void setCellTextColor(int x, int y, GameSnake.Color color) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    public GameSnake.Color getCellTextColor(int x, int y) {
        ObservableList<Node> children = this.cells[y][x].getChildren();
        if (children.size() > 1) {
            Text text = (Text)children.get(1);
    ...
    private GameSnake.Color fromFXColor(Color color) {
        ...
        GameSnake.Color[] result = new GameSnake.Color[]{GameSnake.Color.NONE};
        optionalColorEntry.ifPresent((entry) -> {
            result[0] = GameSnake.Color.valueOf((String)entry.getKey());
        });
        return result[0];
    }

    private Color toFXColor(GameSnake.Color color) {
        if (color == GameSnake.Color.NONE) {
    ...
    public void setCellValueEx(int x, int y, GameSnake.Color cellColor, String value) {
        this.setCellValue(x, y, value);
        this.setCellColor(x, y, cellColor);
    }

    public void setCellValueEx(int x, int y, GameSnake.Color cellColor, String value, GameSnake.Color textColor) {
        this.setCellValueEx(x, y, cellColor, value);
        this.setCellTextColor(x, y, textColor);
    }

    public void setCellValueEx(int x, int y, GameSnake.Color cellColor, String value, GameSnake.Color textColor, int textSize) {
        this.setCellValueEx(x, y, cellColor, value, textColor);
        this.setCellTextSize(x, y, textSize);
    }

    public void showMessageDialog(GameSnake.Color cellColor, String message, GameSnake.Color textColor, int textSize) {
    ...
    public void setLives(int lives) {
        if (this.livesText == null) {
            ...
            this.livesText.setFill(this.toFXColor(GameSnake.Color.ORANGE));
